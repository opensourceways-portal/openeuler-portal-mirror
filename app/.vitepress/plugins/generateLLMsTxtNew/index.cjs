...
function viteLlmsTxt(options) {
  ...
  return {
    ...
    closeBundle() {
      ...
      const outDir = options.outDir ? (0, import_node_path2.resolve)(config.root, options.outDir) : (0, import_node_path2.resolve)(config.root, config.build.outDir);
      const content = generateContent(options);
      (0, import_node_fs2.mkdirSync)(outDir, { recursive: true });
      (0, import_node_fs2.writeFileSync)((0, import_node_path2.join)(outDir, filename), content, "utf-8");
      config.logger.info(
        `\x1B[32m\u2713\x1B[0m \x1B[2m[llms-txt]\x1B[0m generated \x1B[36m${(0, import_node_path2.join)(outDir, filename)}\x1B[0m`
      );
    }
  };
}
