...
SBOM数模设计解耦于SPDX、CycloneDX等主流SBOM标准协议，将SBOM拆分为各个元素，支持导出
不同协议格式的SBOM

### 3.4.1 整体数模框架

以制品（product）为主体，一对一关联SBOM，每个SBOM关联若干组件，组件卷积License和漏洞
![](./data-model-overall.png)

### 3.4.2 制品及其统计分析数模

记录制品（product）基本信息与统计分析数据
![](./data-model-product.png)

### 3.4.3 组件包数模

记录组件包（package）基本信息与统计数据，通过关联表与License和漏洞关联
![](./data-model-pkg.png)

### 3.4.4 License数模

记录License信息，及与组件包的关联关系
![](./data-model-license.png)

### 3.4.5 漏洞数模

记录漏洞信息、引用与评分等，及与组件包的关联关系
![](./data-model-vuln.png)
